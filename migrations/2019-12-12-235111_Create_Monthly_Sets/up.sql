-- Your SQL goes here
CREATE OR REPLACE PROCEDURE bb.generate_monthly_set(top_n_cards INT)
AS $$
BEGIN
  with
    inserted_set AS (
      INSERT INTO bb.parent_set (name)
        VALUES ('Montly Set: ' || TO_CHAR(NOW()::DATE, 'yyyy-mm'))
      RETURNING id)
    , card_ratings AS (
      SELECT
        r.card_id
    ...
    , top_cards AS (
      ...
      WHERE DATE_PART('month', created_date - NOW()::timestamp) <= 1
