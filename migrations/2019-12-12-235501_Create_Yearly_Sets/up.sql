-- Your SQL goes here
CREATE OR REPLACE PROCEDURE bb.generate_yearly_set(top_n_cards INT)
AS $$
BEGIN
  with
    inserted_set AS (
      INSERT INTO bb.parent_set (name)
        VALUES ('Yearly Set: ' || TO_CHAR(NOW()::DATE, 'yyyy'))
      RETURNING id)
    , card_ratings AS (
      SELECT
        r.card_id
    ...
    , top_cards AS (
      ...
      WHERE DATE_PART('year', created_date - NOW()::timestamp) <= 1
