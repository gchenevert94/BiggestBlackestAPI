  ...
  IF get_random THEN
    ...
    RETURN QUERY SELECT
      ...
      WHERE
        ...
        AND (
          -- NULL == CardSource.All
          user_submitted IS NULL OR
          -- TRUE == CardSource.User
          (user_submitted AND c.submitted_by_user_id IS NOT NULL) OR
          -- FALSE == CardSource.Official
          (NOT user_submitted AND c.submitted_by_user_id IS NULL)
        )
      ORDER BY RANDOM()
